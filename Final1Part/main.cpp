...
void Gen(int min,int max) {
    size_t number = std::chrono::time_point_cast<std::chrono::microseconds>(
    ...
    std::uniform_int_distribution<> size(0,n-1);
    std::uniform_int_distribution<> value(min,
                                          max);
    vector<string> docs(value(gen));
    for (auto &x:docs) {
        for (int j=0;j<value(gen);++j) x+=words[size(gen)]+' ';
    }
    vector<string> queries(value(gen));
    for (auto &x:docs) x=words[size(gen)];
    TestFunctionality(docs,words,docs);
}
