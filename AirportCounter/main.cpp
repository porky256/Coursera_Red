...
public:
    ...
    AirportCounter():size(static_cast<size_t>(TAirport::Last_)) {
        values.fill(0);
    };

    // конструктор от диапазона элементов типа TAirport
    template <typename TIterator>
    AirportCounter(TIterator begin, TIterator end):size(static_cast<size_t>(TAirport::Last_)){
        values.fill(0);
        for (auto iter = begin;iter<end;++iter) {
            values[static_cast<uint32_t>(*iter)]++;
        }
    };

    // получить количество элементов, равных данному
    size_t Get(TAirport airport) const {
        return values[static_cast<uint32_t>(airport)];
    }

    // добавить данный элемент
    void Insert(TAirport airport) {
        values[static_cast<uint32_t>(airport)]+=1;
    };
    ...
    void EraseAll(TAirport airport) {
        values[static_cast<uint32_t>(airport)]=0;
    };
    ...
    Items GetItems() const {
        Items x;
        for (int i=0;i<size;++i) x[i]=make_pair(static_cast<TAirport>(i),values[i]);
        return x;
    };

private:
    const uint32_t size;
    array<uint32_t ,static_cast<uint32_t>(TAirport::Last_)> values;
};
